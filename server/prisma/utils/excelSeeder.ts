...
type ModelName = 'inverter' | 'pVModule' | 'energyStorageSystem';

type ModelInputType = {
  inverter: Prisma.InverterCreateManyInput;
  pVModule: Prisma.PVModuleCreateManyInput;
  energyStorageSystem: Prisma.EnergyStorageSystemCreateManyInput;
};
...
const fileConfig: Record<ModelName, { skipRows: number }> = {
  inverter: { skipRows: 14 },
  pVModule: { skipRows: 16 },
...
const fieldMappings: Record<ModelName, ExcelMapping[]> = {
  inverter: [
    ...
    { excelField: 'Equipment Type', modelField: 'equipmentType' },
    { excelField: 'Description', modelField: 'description' },
    { excelField: 'Phase Type', modelField: 'phaseType' },
...
function transformExcelData<T extends ModelName>(
  ...
  return data
    .map((row: any, index: number) => {
      ...
      if (modelName === 'pVModule' && (!transformedRow.manufacturer || !transformedRow.modelNumber)) {
        return null;
      }
  ...
  try {
    ...
    switch (modelName) {
      case 'inverter':
        ...
        break;
      case 'pVModule':
        result = await prisma.pVModule.createMany({
          data: transformedData as Prisma.PVModuleCreateManyInput[],
          skipDuplicates: true
        });
        break;
...
async function seedFromExcel() {
  try {
    ...
    await prisma.pVModule.deleteMany();
    ...
    await importExcelToDatabase(
      path.join(seedDataDir, 'PVModule.xlsx'),
      'pVModule'
    );

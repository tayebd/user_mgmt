...
async function main() {
  fs.createReadStream(path.resolve(__dirname, "seeds/Organization.csv"))
    ...
    .transform(async (row: any, next) => {
      ...
      // If organization data exist update it, otherwise create it
      const res = await prisma.organization.upsert({
        where: { name: row.name },
        update: {
          address: row.address || '',

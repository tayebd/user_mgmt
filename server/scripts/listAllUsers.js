...
async function listAllUsers() {
    ...
    try {
        ...
        try {
            ...
            console.table(supabaseUsers.users.map((user) => ({
                id: user.id,
            ...
            const supabaseIds = new Set(supabaseUsers.users.map((user) => user.id));
            ...
            if (missingInDb.length > 0) {
                ...
                for (const id of missingInDb) {
                    const sbUser = supabaseUsers.users.find((u) => u.id === id);
                    console.log(`- ${(sbUser === null || sbUser === void 0 ? void 0 : sbUser.email) || 'N/A'} (${id})`);
                }
            }
            else {
            ...
            if (missingInSupabase.length > 0) {
                ...
                for (const id of missingInSupabase) {
                    ...
                    console.log(`- ${(dbUser === null || dbUser === void 0 ? void 0 : dbUser.email) || 'N/A'} (${id})`);
                }
            }
            else {
            ...
            for (const dbUser of dbUsers) {
                const sbUser = supabaseUsers.users.find((u) => u.id === dbUser.uid);
